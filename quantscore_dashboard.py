        ...
        try:
            ...
            try:
                ...
                if not intraday.empty:
                    ...
                    gap_pct = change_pct  # Fallback to daily change
            except:
                gap_pct = change_pct  # Fallback to daily change
            
            # Calculate RSI
            hist_for_rsi = stock.history(period="30d", interval="1d")
            if len(hist_for_rsi) >= 14:
                rsi = ta.momentum.RSIIndicator(hist_for_rsi['Close']).rsi().iloc[-1]
            else:
                rsi = 50  # Default if insufficient data
...
def calculate_quant_score(data: Dict, alpha: float = 1.3, beta: float = 0.7) -> float:
...
def apply_filters(df: pd.DataFrame, rsi_min: float, gap_min: float, max_float: int) -> pd.DataFrame:
...
def format_number(num: float, is_currency: bool = False, is_percentage: bool = False) -> str:
...
def create_performance_chart(df: pd.DataFrame):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('QuantScore Distribution', 'Change% vs Volume', 'RSI Distribution', 'Market Cap vs Float'),
    ...
    fig.add_trace(
        go.Histogram(x=df['quant_score'], name='QuantScore', nbinsx=20),
        row=1, col=1
    )
    
    # Change% vs Volume scatter
    fig.add_trace(
        go.Scatter(
            x=df['volume'], 
            y=df['change_pct'],
            ...
            text=df['ticker'],
            name='Stocks',
            marker=dict(size=8, color=df['quant_score'], colorscale='Viridis')
        ),
        row=1, col=2
    )
    ...
    # RSI distribution
    fig.add_trace(
        go.Histogram(x=df['rsi'], name='RSI', nbinsx=15),
        row=2, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=df['ticker'],
            name='Cap vs Float',
            marker=dict(size=8, color=df['quant_score'], colorscale='Plasma')
        ),
        row=2, col=2
    )
...
def main():
    ...
    # Default high-potential tickers (mix of sectors)
    default_tickers = [
    ...
    gap_min = st.sidebar.slider("Minimum Gap %", 0.0, 10.0, 2.0, 0.5)
    ...
    if st.sidebar.button("🚀 Run QuantScore Analysis", type="primary") or auto_refresh:
        ...
        # Apply filters
        filtered_df = apply_filters(df, rsi_min, gap_min, max_float)
        ...
        with col4:
            if not filtered_df.empty:
                avg_gap = filtered_df['gap_pct'].mean()
                st.metric(
                    label="Avg Gap %",
                    value=f"{avg_gap:.2f}%",
                    delta=f"Max: {filtered_df['gap_pct'].max():.2f}%"
                )
            else:
        ...
        if filtered_df.empty:
            ...
            st.info("💡 Try lowering the RSI minimum or Gap minimum to see more results.")
        ...
        display_formatted.columns = [
            ...
            'Market Cap', 'Float Shares', 'Gap%', 'RSI'
        ]
        
        # Display the table with ranking
        display_formatted.index = range(1, len(display_formatted) + 1)
        display_formatted.index.name = 'Rank'
        ...
        with col3:
            st.info(f"💹 Market Status: {'🟢 Open' if 9 <= datetime.now().hour <= 16 else '🔴 Closed'}")
        
        # Performance analytics
        if len(display_df) >= 3:
            st.markdown("## 📊 Performance Analytics")
            with st.spinner("Creating performance charts..."):
                chart = create_performance_chart(display_df)
                if chart:
        ...
        with st.expander("🧮 QuantScore Formula & Methodology"):
            ...
            - **Gap%:** > {gap_min}% (breakout potential)
            ...
            - **Red metrics** = Areas of concern
            
            ### Data Sources:
            - Yahoo Finance API (real-time quotes)
            - Technical Analysis library (RSI calculations)
        ...
        # Sample screenshot or demo data could go here
